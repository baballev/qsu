...
class ReplayMemory2(object): # TODO: inheritance correctly ....
    ...
    def push(self, *args):
        if len(self.memory) < self.capacity:
            self.memory.append(None)
        self.memory[self.position] = Transition2(args[0].squeeze(0), args[1].squeeze(0), args[2], args[3].squeeze(0))
        self.position = (self.position + 1) % self.capacity
...
blank_trans = (0, np.zeros((150, 95), dtype=np.uint8), 0, 0.0, False)
...
class PrioritizedMemory:
    def __init__(self, capacity, history_length=4, discount=0.999, multi_step=3, priority_weight=0.4, priority_exponent=0.5):
        self.device = device
        self.capacity = capacity
