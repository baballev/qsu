...
def log_episodes(i, q_trainer, steps, delta_t):
    if i <= 1 or i % 30 == 0:
...
def trainQNetwork(episode_nb, learning_rate, batch_size=BATCH_SIZE, load_weights=None, save_name='tests',
                  ...
                  initial_p=1.0, end_p=0.05, decay_p=2000000, target_update=30000):
    training_steps = 0
    print_infos()
    if evaluation:
        learning_rate = 0.0
    ...
    q_trainer = QTrainer(env, batch_size=batch_size, lr=learning_rate, gamma=GAMMA, initial_p=initial_p, end_p=end_p, decay_p=decay_p, load_weights=load_weights, load_memory=load_memory)
    ...
    for i in range(episode_nb):
        ...
        log_episodes(i, q_trainer, steps, delta_t)
